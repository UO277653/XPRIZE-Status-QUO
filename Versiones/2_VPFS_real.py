...
def ansatz(params):
    """
    Calcula el vector v a partir del vector de parámetros params.

    Args:
        params (list or numpy.ndarray): Vector de parámetros.

    Returns:
        list: Vector v normalizado.
    """
...
def create_circuits(Y, B, num_qubits):
    """
    Crear los circuitos qc1 y qc2 sin inicialización.
    """
    dim = 2 ** num_qubits
...
def calculate_loss_with_simulation(params, qc1, qc2, simulator, num_qubits, B_norm, Y_norm):
    ...
    try:
        ...
        init_circuit.initialize(v, list(range(num_qubits, 2 * num_qubits)))  # Inicializar segundo registro
    ...
    statevector1 = np.array(result1.data(0)["statevector_yv"])  # Recuperar estado vectorial como array
    ...
    statevector2 = np.array(result2.data(0)["statevector_ub"])  # Recuperar estado vectorial como array
    ...
    loss.append(1 - (norm_after_ub)**2/ norm_yv_cnot**2 + pen_coef * (norm_YV_cnot - B_norm)**2)
    loss.append(1 - (norm_after_ub)**2/ norm_yv_cnot**2 + pen_coef * (norm_YV_cnot - B_norm)**4)
    a2 = norm_YV_cnot**2
    b2 = B_norm**2
    ab = norm_after_ub * Y_norm * B_norm * V_norm * V_norm
    loss.append((a2-ab)**2+(b2-ab)**2)#a2**2+b2**2+2*ab**2-2*ab*(a2+b2)
    loss.append(a2+b2-2*ab)
    # calculos = [a2,b2,ab,loss2,loss3]
...
def finite_difference_gradient(params, qc1, qc2, simulator, num_qubits, B_norm, Y_norm, delta=1e-4):
    ...
    for i in range(len(params)):
        ...
        loss_plus = calculate_loss_with_simulation(params_plus, qc1, qc2, simulator, num_qubits, B_norm, Y_norm)
        loss_minus = calculate_loss_with_simulation(params_minus, qc1, qc2, simulator, num_qubits, B_norm, Y_norm)
        grad[i] = (loss_plus - loss_minus) / (2 * delta)
    return grad
def finite_difference_gradient_sequential(params, qc1, qc2, simulator, num_qubits, B_norm, Y_norm, delta=1e-4):
    """
...
def quantum_optimization_simulation(num_qubits=2, ansatz_params=None, optimizer="basic"):
    ...
    if optimizer == "basic":  # Tasa de aprendizaje
        for iter in range(max_iters):
            loss = calculate_loss_with_simulation(ansatz_params, qc1, qc2, simulator, num_qubits, B_norm, Y_norm)
            # if iter % 100 == 0:
            #     v = ansatz(ansatz_params)
            #     v0 = v[0]
            #     Vsol = [x / v0 for x in v]
            #     print(f"Iter {iter + 1}: Loss = {loss:.6f}, Params = {ansatz_params}, V = {Vsol}")
            if loss == None:
                print("NO CONVERGIÓ!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                loss = - 100
                break
            ...
            grad = finite_difference_gradient(ansatz_params, qc1, qc2, simulator, num_qubits, B_norm, Y_norm)
            # if loss < loss_th:
            #     ansatz_params -= learning_rate * grad * loss/loss_th
            # else:
        ...
        for iter in range(max_iters):
            ...
            if loss == None:
                ...
                break
            ansatz_params = finite_difference_gradient_sequential(ansatz_params, qc1, qc2, simulator, num_qubits, B_norm, Y_norm)
    elif optimizer == "cobyla":
        def loss_func(params):
            loss= calculate_loss_with_simulation(params, qc1, qc2, simulator, num_qubits, B_norm, Y_norm)
            return loss
            # return calculate_loss_with_simulation(params, qc1, qc2, simulator, num_qubits, B_norm, Y_norm) 
